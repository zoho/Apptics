...
//        playerLayer.frame = CGRect(x: widthOfCornerView/2.0, y: heightOfCornerView/2.0 , width: frame.size.width - widthOfCornerView, height: frame.size.height - heightOfCornerView)
    ...
    private func addTimeObserver() {
        ...
        let labelObserverBlock: (CMTime) -> Void = { [weak self] _ in
        ...
        self.labelObserver = player.addPeriodicTimeObserver(forInterval: labelUpdatingInterval, queue: .main, using: labelObserverBlock)

        let scrollUpdatingInterval = CMTime(seconds: 0.1, preferredTimescale: CMTimeScale(NSEC_PER_SEC))
        let scrollObserverBlock: (CMTime) -> Void = { [weak self] _ in
            guard let self = self else { return }
            if self.isSeekedToEnd { return } // ✅ Explicit self
            let currentPlayBackTime = self.player.currentTime()
            let playerTime = CMTimeGetSeconds(currentPlayBackTime) < 0 ?
                CMTimeMakeWithSeconds(0, preferredTimescale: currentPlayBackTime.timescale) :
                currentPlayBackTime
            self.viewModel.updateScroll(time: playerTime, withTotalDuration: self.totalVideoPlayBackDuration)
        }
        self.scrollObserver = player.addPeriodicTimeObserver(forInterval: scrollUpdatingInterval, queue: .main, using: scrollObserverBlock)
    }
...
extension ViewPlayerView: VideoPlayerViewProtocol{
    ...
    func seek(to time: CMTime) {
        self.isSeekedToEnd = (time == totalVideoPlayBackDuration)
        if self.isSeekedToEnd{

...
extern AppticsDC const AppticsDCJP NS_SWIFT_NAME(JP);
extern AppticsDC const AppticsDCCN NS_SWIFT_NAME(CN);
extern AppticsDC const AppticsDCAE NS_SWIFT_NAME(AE);
extern AppticsDC const AppticsDCUK NS_SWIFT_NAME(UK);



@interface MultipleDC : NSObject

// activeDC is no longer a property — the active datacenter is persisted
// directly in NSUserDefaults under key "ap_apptics_activeDC" (see ZAPIIManager /
// ZASystemInfo / APDBManager). defaultDC below is still developer-set config.
@property (nonatomic, strong) AppticsDC _Nullable defaultDC;
@property (nonatomic, assign) BOOL preferUserDCForEngagements;

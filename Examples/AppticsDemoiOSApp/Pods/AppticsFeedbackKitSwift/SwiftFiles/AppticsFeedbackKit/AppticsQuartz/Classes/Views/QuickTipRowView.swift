...
class QuickTipRowView: UIView{
    ...
    private let creatorHelpDocumentationUrlStr = "https://help.zoho.com/portal/en/kb/creator/developer-guide/others/zia-help-assistant/articles/introducing-quartz-for-creator"
    ...
    init(text: String, helpDocUrlRefStr: String? = nil) {
        self.tipText = text
        self.helpDocUrlStr = helpDocUrlRefStr
    ...
    func makeTappableLink(content: String){
        guard let helpDocumentationUrl = URL(string: helpDocUrlStr ?? creatorHelpDocumentationUrlStr) else {return}
        ...
        if let primaryColor = QuartzKit.shared.primaryColor{
            ...
            attributedString.addAttribute(.strokeColor, value: QuartzRGBColor.viewDetailsLabel , range: linkRange)
        }

...
class APEventExtension: NSObject, APEventsProtocol {
	...
	class func formatType(toPrivateObject group: String, event: String) -> APPrivateObject? {
		...
		let event_str = "\(group)_\(event)".lowercased()
...
class APAPAPIExtension: NSObject, APAPIProtocol {
	...
	class func directMatch(_ url: String) -> String? {
		...
		}else if url == "http://numbersapi.com/random/math" {
			return "2138768520939"
		}else if url == "https://list.ly/api/v4/meta?url=http://www.zoho.com" {
			return "2138768572991"
		}else if url == "https://ip-fast.com/api/ip/" {
	...
	class func patternMatch(_ url: String) -> String? {
		...
		}else if self.isMatch(url, with:"https://www.7timer.info/bin/astro.php") == true{
